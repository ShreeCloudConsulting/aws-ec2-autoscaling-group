...
resource "aws_autoscaling_policy" "scale_up" {
  ...
  name                   = "${var.environment}-${var.application}-asg-scale-up"
...
resource "aws_autoscaling_policy" "scale_down" {
  count                  = local.autoscaling_enabled ? 1 : 0
  name                   = "${var.environment}-${var.application}-asg-scale-down"
...
locals {
  default_ec2_alarms = {
    cpu_high = {
      alarm_name                = "${var.environment}-${var.application}-cpu-utilization-high"
    ...
    cpu_low = {
      alarm_name                = "${var.environment}-${var.application}-cpu-utilization-low"
...
resource "aws_cloudwatch_metric_alarm" "all_alarms" {
  ...
  alarm_name = format("%s%s", "{var.environment}-${var.application}", each.value.alarm_name)
  #alarm_name               = format("${var.environment}-${var.application}", each.value.alarm_name)
